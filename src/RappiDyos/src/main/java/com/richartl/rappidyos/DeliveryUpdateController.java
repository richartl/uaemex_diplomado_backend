 ...
 * @author richartl
 */
@WebServlet(name = "DeliveryUpdateController", urlPatterns = {"/DeliveryUpdateController"})
public class DeliveryUpdateController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet DeliveryUpdateController</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet DeliveryUpdateController at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            request.getRequestDispatcher("/DeliveriesUpdate.jsp").forward(request, response);
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            int product_id = Integer.parseInt(request.getParameter("products_list"));
            Product pr = (Product) pss.getById(product_id);
            int delivery_man_id = Integer.parseInt(request.getParameter("delivery_men_list"));
            DeliveryMan dm = (DeliveryMan) dms.getById(delivery_man_id);
            Delivery dl = new Delivery(dm, pr);
            dl.setId(oldDl.getId());
            dls.update(dl);
