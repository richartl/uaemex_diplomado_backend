 ...
 * @author richartl
 */
@WebServlet(urlPatterns = {"/TransportUpdateController"})
public class TransportUpdateController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet TransportUpdateController</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet TransportUpdateController at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            request.getRequestDispatcher("/TransportsUpdate.jsp").forward(request, response);
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            int transportId = Integer.parseInt(request.getParameter("transport_id"));
            String vin = request.getParameter("vin");
            String transportType = request.getParameter("transport_type");
            Transport oldTs = (Transport) tss.getById(transportId);
            Transport ts = new Transport(vin, transportType);
            ts.setId(oldTs.getId());
            tss.update(ts);
